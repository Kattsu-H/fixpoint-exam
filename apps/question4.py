...
def detect_failuer(input, n, m, t):
    with open(input, "r") as f:
        ...
        logs = [
            [
                ...
                row[1],
                str(ipaddress.ip_network(row[1], strict=False)),
                row[2],
            ]
            for row in reader
        ]

    # 各サブネットごとのipアドレスのステータス一覧の初期化
    subnet_status = dict()
    for _, ip_address, ip_network, _ in logs:
        if ip_network not in subnet_status.keys():
            subnet_status[ip_network] = {
                "is_subnet_timeout": False,
                "is_server_timeout": {ip_address: False},
                "start_timeout_date": None,
                "last_timeout_date": None,
            }
            continue
        subnet_status[ip_network]["is_server_timeout"][ip_address] = False

    last_status = dict()
    for date, ip_address, ip_network, response_time in logs:
        ...
        if response_time == "-":
            ...
            # タイムアウトがn回以上の場合、subnet_statusにタイムアウト状態として記録
            if last_status[ip_address]["timeout_count"] >= n:
                subnet_status[ip_network]["is_server_timeout"][ip_address] = True
            # サブネット内の全ipがタイムアウトした場合、サブネットをタイムアウトとする
            if all(subnet_status[ip_network]["is_server_timeout"].values()):
                if not subnet_status[ip_network]["is_subnet_timeout"]:
                    subnet_status[ip_network]["start_timeout_date"] = date
                    subnet_status[ip_network]["is_subnet_timeout"] = True
                subnet_status[ip_network]["last_timeout_date"] = date
            continue
        ...
        if last_status[ip_address]["is_timeout"]:
            ...
            # サブネットがタイムアウトしていた場合、サブネットの故障期間を標準出力する
            if subnet_status[ip_network]["is_subnet_timeout"]:
                print(
                    f"[SUBNET_DOWN] {ip_network}: "
                    f"{subnet_status[ip_network]['start_timeout_date']} - {date}"
                )
                subnet_status[ip_network]["is_subnet_timeout"] = False
            subnet_status[ip_network]["is_server_timeout"][ip_address] = False
    ...
    for k, v in subnet_status.items():
        if v["is_subnet_timeout"]:
            print(
                f"[SUBNET_DOWN] {k}: "
                f"{v['start_timeout_date']} - {v['last_timeout_date']}"
            )
